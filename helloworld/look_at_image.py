...
def main(backend: str) -> None:
    ...
    try:
        ...
        error_msg = str(e)
        if "Unable to connect" in error_msg or "ZError" in error_msg or "zenoh" in error_msg.lower():
            print("Error: Cannot connect to Reachy Mini robot.")
            print("\nTroubleshooting steps:")
            print("1. Make sure the robot is powered on")
            print("2. Verify both your computer and robot are on the same WiFi network")
            print("3. Check the robot's IP address:")
            print("   - Open http://ROBOT_IP:8000 in your browser (replace ROBOT_IP with actual IP)")
            print("   - You should see the Reachy Dashboard if connected")
            print("4. If connecting to a wireless robot, ensure localhost_only=False is set")
            print("   (This is already set in the code)")
            print("\nIf the robot is running locally, you may need to start the Zenoh daemon.")
        else:
